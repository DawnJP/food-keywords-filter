...
def filter_keywords(df, exclude_keywords):
    if not exclude_keywords:
        return df
    pattern = re.compile(r'(' + '|'.join(map(re.escape, exclude_keywords)) + r')', re.IGNORECASE)
    def is_excluded(row):
        return bool(pattern.search(str(row['Keyword Phrase']))) or bool(pattern.search(str(row['Vietnamese Translation'])))
